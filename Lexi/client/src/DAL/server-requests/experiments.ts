...
export const getExperiment = async (experimentId: string): Promise<ExperimentType> => {
    try {
        const response = await axiosInstance.get(`/${ApiPaths.EXPERIMENTS_PATH}/${experimentId}`);
        return response.data;
    } catch (error) {
        throw error;
    }
